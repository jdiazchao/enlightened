...
public class TERenderer {
    ...
    public void initialize(int w, int h, int xOff, int yOff) {
        ...
        this.xOffset = xOff;
        this.yOffset = yOff;
        StdDraw.setCanvasSize(width * TILE_SIZE, height * TILE_SIZE + yOffset * TILE_SIZE);
     ...
     */
    public void renderFrame(TETile[][] world, String hudText) {
    ...
    private void displayHUD(String text) {
        StdDraw.setPenColor(Color.white);
        Font fontSmall = new Font("Monaco", Font.TRUETYPE_FONT, TEXT_SIZE);

        double yPos = height - 0.1;
        StdDraw.setFont(fontSmall);
        StdDraw.textLeft(xOffset + 1, yPos, text);

        LocalDateTime time = LocalDateTime.now();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("MM/dd[ HH:mm:ss]");
        String formattedTime = time.format(formatter);
        StdDraw.textRight(xOffset + width - 1, yPos, formattedTime);

        StdDraw.text(width / 2, yPos, "Press i/o to adjust light");
    }
}
